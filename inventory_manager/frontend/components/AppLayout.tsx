...
export default function AppLayout({ // Renamed from DashboardLayout
    children,
}: {
    ...
    useEffect(() => {
        console.log("DashboardLayout: Mounted/Updated", pathname);
        if (!isLoading && !user) {
            console.warn("AppLayout: No user found. Redirecting to /login");
            router.push("/login");
        }
    }, [user, isLoading]);
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-100" suppressHydrationWarning>
            ...
            <div className="hidden md:flex flex-col fixed inset-y-0 left-0 w-20 lg:w-64 bg-slate-900 border-r border-slate-800 z-50">
            ...
            <div className="md:pl-20 lg:pl-64 min-h-screen flex flex-col">
                {/* Mobile Header */}
                <header className="md:hidden h-16 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-4 shrink-0 sticky top-0 z-40">
                ...
                <Link
                    href="/scanner"
                    className="fixed bottom-24 right-6 md:bottom-8 md:right-8 p-4 bg-emerald-500 text-slate-950 rounded-full shadow-lg shadow-emerald-500/20 hover:bg-emerald-400 hover:scale-105 transition-all z-50 flex items-center justify-center"
                    title="Open Scanner"
                >
                    <ScanLine size={24} />
                </Link>
